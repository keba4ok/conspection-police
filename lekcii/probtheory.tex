...
\begin{defn}
    Вся же тройка $(\Omega, \FF, \PP)$ называется \hypertarget{n1}{\textcolor{cyan}{\textit{вероятностным пространством}}}.
\end{defn}
...
\begin{exl}
    ...
    Пространство $(\Omega, \FF, \PP)$ \hypertarget{n2}{\textcolor{cyan}{\textit{дискретно}}}, если $\Omega$ не более, чем счётно. $\FF=\Rho(\Omega)$, элементы $\{\omega\}$ также считаем событиями.
    \begin{stat}
...
Также, можно упомянуть про \hypertarget{n3}{\textcolor{cyan}{\textit{равновероятные исходы}}}, из названия понятно, что это. Если $\vert \Omega\vert <\infty$ и $p_\omega=p$ для любого $\omega$, тогда $\PP(A)=\frac{\vert A\vert}{\vert \omega \vert}$. \ 
...
\begin{defn}
    \hypertarget{n4}{\textcolor{cyan}{\textit{Условная вероятность}}}: $\PP_B(A)=\frac{\PP(A\cap B)}{\PP(B)}$ (при $\PP(B)>0$).
\end{defn}
...
\begin{stat}
    \hypertarget{n5}{\textcolor{cyan}{\textit{Формула Байеса}}}. Пусть мы знаем событие $A$, имеется разбиение $(B_n)$, тогда 
    \[
...
\begin{stat}
    \hypertarget{n6}{\textcolor{cyan}{\textit{Формула умножения}}}. 
    \[
...
\begin{defn}
    События $A_1, \dots, A_n$ \hypertarget{n7}{\textcolor{cyan}{\textit{независимы}}}, если выполнено:
    \begin{align*}
...
\begin{defn}
    \hypertarget{n8}{\textcolor{cyan}{\textit{Случайная величина}}} - это функция $X:\Omega\rightarrow R$. 
\end{defn}
...
\begin{defn}
    Таки образом, совокупность последовательностей $\frac{\{a_k\}}{\{p_k\}}$ и называется \hypertarget{n9}{\textcolor{cyan}{\textit{распеределением случайной величины}}}. 
\end{defn}
...
\begin{exl}
    ...
    \begin{itemize}
        ...
        \item \hypertarget{n10}{\textcolor{cyan}{\textit{биномиальное}}}: $B(n, p)$, $p\in[0,1]$, $X\sim B(n, p)$, если принимаются значения от $0$ до $n$, причём $\PP\{X=k\}=\PP_n(k)$ (просто обозначение), и равно $C_n^kp^k(1-p)^{n-k}$.
    \end{itemize}
